...
API_KEY = "y6WrNQEbTI4iKhXXv3Gtfe3E8GVfgKf6f2A4vvjfGvg"
...
for product in products:
    ...
    response = requests.put(f"{BASE_URL}/{product_id}", headers=headers, data=data)
    result = response.json()
    name = result.get("product", {}).get("name", product_id) if result.get("success") else product_id
    status = "✓" if result.get("success") else f"✗ {result.get('message', 'unknown error')}"
    print(f"{status}  {name}")
